...
```
完整语法：
{[(index|name)][fill][alignment][width][.precision]specifier}

组成部分：
  index/name   - 参数索引或名称（可选）
  fill         - 填充字符（可选，默认空格）
  alignment    - 对齐方式：< 左对齐，> 右对齐，^ 居中（可选）
  width        - 最小宽度（可选）
  precision    - 浮点数精度（可选）
  specifier    - 格式化类型（s, d, x, X, b, o, c, e, E, *, any, B, Bi）
```
...
| `{c}` | `.{65}` | `"A"` | ASCII 字符 |
| `{e}` | `.{3.14}` | `"3.14e0"` | 科学计数法 |
| `{*}` | `.{@ptrFromInt(0xDEAD)}` | `"u8@dead"` | 指针地址 |
| `{B}` | `.{1024}` | `"1.024kB"` | 十进制内存大小 |
| `{Bi}` | `.{1024}` | `"1KiB"` | 二进制内存大小 |
| `{any}` | `.{.{"a", 1}}` | `"struct { ... }"` | 任意类型 |
| `{d:5}` | `.{42}` | `"   42"` | 宽度5，右对齐 |
| `{d:<5}` | `.{42}` | `"42   "` | 宽度5，左对齐 |
| `{d:^5}` | `.{42}` | `" 42  "` | 宽度5，居中 |
| `{0>5}` | `.{42}` | `"00042"` | 零填充，宽度5 |
| `{d:.2}` | `.{3.14159}` | `"3.14"` | 浮点数精度2位 |
| `{s:.3}` | `.{"hello"}` | `"hel"` | 字符串截断 |
| `{0}` | `.{"A","B"}` | `"A"` | 按索引引用 |
| `{[name]}` | `.{.name="Zig"}` | `"Zig"` | 按名称引用 |